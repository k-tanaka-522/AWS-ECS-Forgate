...
### 1.1 目的
本ドキュメントは、AWS Multi-Account Sample Application（Sample App）のPOC環境におけるAWSインフラ設計を定義します。
...
### 1.3 参照ドキュメント
- [01_Sample App企画書.md](01_Sample App企画書.md)
- [02_Sample App要件定義書.md](02_Sample App要件定義書.md)
...
```
┌────────────────────────────────────────────────────────────────────────┐
│                   Sample App POC環境（Multi-Account構成）                  │
│                                                                          │
│ ┌──────────────────────────────────────────────────────────────────┐   │
...
└────────────────────────────────────────────────────────────────────────┘
    ...
    （管理者 - Admin Service）            （住民 - Public Service）
```
...
**3サービス構成:**
- **Public Service**: 一般ユーザー向けWebアプリケーション（インターネット経由、ALB公開）
- **Admin Service**: 管理者向けダッシュボード（VPN経由のみアクセス）
- **Batch Service**: データ処理・集計バッチ（スケジュール実行）
...
| 項目 | 値 |
|-----|---|
| **名前** | Sample App-POC-TransitGateway |
...
| 項目 | 値 |
|-----|---|
| **名前** | Sample App-POC-ECSCluster-Main |
...
| 項目 | 値 |
|-----|---|
| **タスク名** | Sample App-POC-Backend-Task |
| **起動タイプ** | FARGATE |
...
| 項目 | 値 |
|-----|---|
| **サービス名** | Sample App-POC-Backend-Service |
...
|-----|----------|-------|----------------|
| Sample App-POC-TG-Frontend | HTTP | 8080 | /health |
| Sample App-POC-TG-Backend | HTTP | 8080 | /api/health |
...
**シークレット名:** `Sample App-POC-DB-Secret`
...
| グループ名 | 説明 | 権限 |
|----------|-----|-----|
| Administrators | 管理者（管理者） | 全申請閲覧・審査、マスタ管理 |
| BusinessUsers | 一般ユーザー | 自社申請のみ閲覧・作成 |
...
**ダッシュボード名:** `Sample App-POC-Dashboard`
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:secretsmanager:*:*:secret:Sample App-*"
    }
  ]
}
```
...
```json
{
  ...
  "Statement": [
    ...
    },
    {
      ...
      "Resource": "arn:aws:secretsmanager:*:*:secret:Sample App-POC-DB-Secret-*"
    }
  ]
}
```
...
```
infra/platform/
├── network.yaml（Sample App-POC-Platform-Network）
│   ├── Shared VPC
│   ├── Transit Gateway
│   └── Client VPN
├── parameters-poc.json
...
```
infra/service/
├── 01-network.yaml（Sample App-POC-Network）
│   ├── Service VPC
...
│
├── 02-database.yaml（Sample App-POC-Database）
│   ├── RDS PostgreSQL
│   ├── DB Subnet Group
│   └── Secrets Manager
│
├── 03-compute.yaml（Sample App-POC-Compute）
│   ├── ECS Cluster
...
│   └── Cognito User Pool
│
├── 04-monitoring.yaml（Sample App-POC-Monitoring）
