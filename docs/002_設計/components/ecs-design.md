...
#### 2.2.1 マルチステージビルド
効率的なコンテナイメージを作成するためのマルチステージビルドを採用します：
...
- **タグ戦略**：
  - 開発環境：コミットハッシュとブランチ名（例：`dev-f8a7b9c`）
  - ステージング環境：リリース候補バージョン（例：`rc-1.0.0`）
  - 本番環境：セマンティックバージョニング（例：`1.0.0`）
- **イメージスキャン**：ECRの自動スキャン機能を有効化し、脆弱性を検出
- **ライフサイクルポリシー**：30日以上経過した未使用イメージを自動削除
