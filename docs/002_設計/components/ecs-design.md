...
##### 2.2.2.6 Java 11ランタイム選択の理由

Spring Bootアプリケーションの実行環境として、以下の理由からJava 11ランタイムを採用しています：

1. **長期サポート（LTS）バージョン**: Java 11はLTSバージョンであり、長期間のセキュリティパッチと安定したサポートが提供されます。

2. **Spring Boot 2.7.xとの互換性**: プロジェクトで使用しているSpring Boot 2.7系はJava 11との互換性が高く、安定した動作とパフォーマンスを提供します。

3. **モダンな言語機能とAPI**: Java 11には、Java 8と比較して多くの改善（HTTPクライアント、Stringメソッドの拡張、モジュールシステムなど）があり、開発効率と保守性が向上します。

4. **AWSサービスとの互換性**: AWSの多くのマネージドサービスでJava 11が広くサポートされており、ECS Fargateでの実行に最適です。

5. **コンテナリソース効率**: Java 11はコンテナ環境に対する最適化が施されており、リソース使用量の認識やメモリ管理が改善されています。これにより、Fargateでのコンテナ実行効率が向上します。

なお、ベースイメージとして採用するAmazon Corretto 11は、パフォーマンスとセキュリティに優れたAWS提供のOpenJDK互換実装であり、AWSサービスとの相性も良好です。
